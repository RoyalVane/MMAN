    ...
    if opt.lr_policy == 'lambda':
        if opt.which_model_netG == 'deeplab_aspp':
            ...
            def lambda_rule(epoch):
                ...
                lr_l = 0.1 ** max(0.0, epoch//50.0)
                #lr_l = 0.1 ** max(1.0, epoch//3.0 + 1.0)
                return lr_l
...
def define_G(input_nc, output_nc, ngf, which_model_netG, hook, input_size, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]):
    ...
    elif which_model_netG == 'pose':
        netG = UnetGenerator2(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids)
...
def define_D(input_nc, ndf, which_model_netD,
    ...
    elif which_model_netD == 'n_layers' or n_layers_D >= 5:
        netD = NLayerDiscriminator_label(input_nc, 20, 64, n_layers_D, norm_layer=norm_layer, use_sigmoid=use_sigmoid, gpu_ids=gpu_ids)
    else:
...
class GANLoss_label(nn.Module):
    def __init__(self, tensor=torch.FloatTensor):
        super(GANLoss_label, self).__init__()
        self.loss = nn.MSELoss()
...
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, hook, ngf=64, 
        ...
        T_block = UnetSkipConnectionBlock(output_nc, ngf * 32, input_nc = None, submodule = None, depth = -3, norm_layer = norm_layer, model_ft = model_res101)
        handle = T_block.register_forward_hook(hook.hook_out)
        U_block = UnetSkipConnectionBlock(output_nc, ngf * 32, input_nc = None, submodule = T_block, depth = -2, norm_layer = norm_layer, model_ft = model_res101) 
        U_block = UnetSkipConnectionBlock(ngf * 32, ngf * 32, input_nc = None, submodule = U_block, depth = -1, norm_layer = norm_layer, model_ft = model_res101) 
...
# =============================================================================
#             conv_S = nn.Conv2d(4, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
#             conv_S.weight.data.normal_(0, 0.01)
#             ResBlock0 = [conv_S, model_ft.bn1]
# =============================================================================
            conv_S = nn.Conv2d(4, 3, kernel_size=1, stride=1, bias=False)
            conv_S.weight.data.normal_(0, 0.01)
            ResBlock0 = [conv_S, model_ft.conv1, model_ft.bn1]
        ...
        conv1 = nn.Conv2d(inner_nc, int(inner_nc/4), kernel_size=1)
        conv_list.append(conv1)
        
        for n in range(1, 4):
            conv3 = nn.Conv2d(inner_nc, int(inner_nc/4), kernel_size=3)
            conv3.stride = (1, 1)
            conv3.dilation = (n, n)
            conv3.padding = (n, n)
        ...
        if depth == -2: #idiot layer, forwards x straightly to next Unet block
            model = [submodule]
            self.U_2 = nn.Sequential(*model)
            
        if depth == -3: #model(x) forwards to Hook 
            model = [nn.Conv2d(inner_nc, outer_nc, 
                              kernel_size=3, stride=1,
                              padding=1, bias=use_bias)]
            self.U_3 = nn.Sequential(*model)
...
class NLayerDiscriminator_label(nn.Module):
    def __init__(self, input_nc, output_nc, ndf=64, n_layers=6, norm_layer=nn.BatchNorm2d, use_sigmoid=False, gpu_ids=[]):
        super(NLayerDiscriminator_label, self).__init__()
        self.gpu_ids = gpu_ids
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
        ...
        sequence += [nn.Conv2d(ndf * nf_mult, output_nc, kernel_size=kw)]
