...
def construct_pose_model(pose_model_path):

    model = pose_model.PoseModel(num_point=19, num_vector=19)
    state_dict = torch.load(pose_model_path)['state_dict']
...
def normalize(origin_img):
    ...
    origin_img = (std * origin_img + mean) * 255
...
#def draw_tensor(array, point):
    

def process(model, image, shrink):

    #origin_img = cv2.imread(str(input_path[0]))
    origin_img = image.numpy()
    origin_img = np.transpose(np.squeeze(origin_img), (1, 2, 0))
    ...
    # draw points
    total_num = 0
    pose_map = np.zeros((int(height/shrink), int(width/shrink), 19)) # num_point
    for i in range(18):
        for j in range(len(all_peaks[i])):
            pose_map[int(all_peaks[i][j][1]/shrink), int(all_peaks[i][j][0]/shrink), i] = 1
            total_num = total_num + 1
#            pose_map[min(all_peaks[i][j][1] + 1, 255), all_peaks[i][j][0], i] = 1
#            pose_map[max(all_peaks[i][j][1] - 1, 0), all_peaks[i][j][0], i] = 1
#            pose_map[all_peaks[i][j][1], min(all_peaks[i][j][0] + 1, 255), i] = 1
#            pose_map[all_peaks[i][j][1], max(all_peaks[i][j][0] - 1, 0), i] = 1
            
    pose_map = np.transpose(pose_map, (2, 0, 1)) # (c, h, w)
    pose_map = torch.from_numpy(pose_map)
    pose_map = torch.unsqueeze(pose_map, 0) # (b, c, h, w)

    return {'pose_map': pose_map, 'total_points': total_num}
...
if __name__ == '__main__':
    ...
    # load model
    model = construct_pose_model(args.model)

    tic = time.time()
    print('start processing...')

    # generate image with body parts
    canvas = process(model, input_image, 256)['canvas']
    heatmap = process(model, input_image, 256)['heatmap']
    toc = time.time()
